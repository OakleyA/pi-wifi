...
const commands = {
    ...
    startSupplicant:
        "sudo wpa_supplicant -Dwext -c :CONFIG_FILE -B -i :INTERFACE  && sudo chattr -i :DNS_FILE",
...
function scan(callback) {
    tools.wpa.scan(currentInterface, function(err, data) {
        if (err || !data.hasOwnProperty("result") || data.result !== "OK")
...
function setCurrentInterface(iface, callback) {
    exec(
        replaceInCommand(commands.wpaInterface, { wifiInterface: iface }),
        function(err) {
...
function prepareConnectionDetails(details) {
    var params = {};
    if (details.hasOwnProperty("ssid"))
        params.ssid = "'\"" + details.ssid + "\"'"; //Add ssid

    if (!details.hasOwnProperty("password")) {
        if (!details.hasOwnProperty("key_mgmt"))
            params.key_mgmt = defaultOpenKey; //Set key_mgmt for an open network
    } else if (!details.hasOwnProperty("username")) {
        //If no user then it is a regular secured network
        params.psk = "'\"" + details.password + "\"'";
    } else {
        //If it has password and user it must be an enterprise network
    ...
    if (details.hasOwnProperty("phase1"))
        params.phase1 = "'\"" + details.phase1 + "\"'";
    if (details.hasOwnProperty("phase2"))
        params.phase2 = "'\"" + details.phase2 + "\"'";
...
function connection(details, callback) {
    ...
    findConnection(details.ssid, function(err, networkId) {
        //Look for an existing connection with that SSID
        async.series(
            {
                remove: function(next) {
                    //Remove the network if found
                ...
                setup: function(next) {
                    //Add provided parameters to network
                    setupConnection(netId, params, next);
                }, //Can't use async.apply due to netId being shared
                start: function(next) {
                    connectToId(netId, next);
                } //Actually connect to the network
            },
            function(err, results) {
                if (err && results.create)
                    tools.wpa.remove_network(currentInterface, netId, callback);
                else callback(err);
            }
        );
    });
...
function setupConnection(netId, params, callback) {
    async.eachOf(
        ...
        function(value, key, next) {
            //For each one of the parameters listed
 ...
 * @param {string} wifiInterface Interface to use
 ...
 */
function setNetworkParameter(wifiInterface, networkId, name, value, callback) {
    tools.wpa.set_network(wifiInterface, networkId, name, value, callback);
}
...
function checkConnection(ssid, cb) {
    ...
    status(function(err, status) {
        if (!err) {
            ...
            if (
                status.hasOwnProperty("ssid") &&
                status.hasOwnProperty("wpa_state")
            ) {
                result.selected = status.ssid === ssid;
                if (result.selected)
                    result.connected = status.wpa_state === "COMPLETED";
...
function connectToId(networkId, callback) {
    ...
    tools.wpa.enable_network(currentInterface, networkId, function(err, data) {
        ...
        tools.wpa.select_network(currentInterface, networkId, function(
            err,
            data
        ) {
            if (err || !data.hasOwnProperty("result") || data.result !== "OK")
            ...
            for (var i = 0; i < lines.length; i++) {
                if (lines[i].indexOf("wlan") > -1) {
                    ...
                    if (options.indexOf("-i") > -1)
                        iface = options[options.indexOf("-i") + 1];
                    if (options.indexOf("-c") > -1)
                        configFile = options[options.indexOf("-c") + 1];
                    result = true;
                }
            }
        }
...
function startSupplicant(options, callback) {
    ...
    var iface = options.hasOwnProperty("iface")
        ? options.iface
        : currentInterface;
    var configFile = options.hasOwnProperty("config")
        ? options.config
        : defaultSupplicantConfigFile;
    var dnsFile = options.hasOwnProperty("dns") ? options.dns : defaultDNSFile;
    exec(
        replaceInCommand(commands.startSupplicant, {
...
function interfaceUp(iface, callback) {
    ...
    exec(
        replaceInCommand(commands.interfaceUp, { wifiInterface: iface }),
        function(err) {
            callback(err);
        }
    );
...
function interfaceDown(iface, callback) {
    iface = iface || currentInterface;
    exec(
        replaceInCommand(commands.interfaceDown, { wifiInterface: iface }),
        function(err) {
            callback(err);
        }
    );
...
function restartInterface(iface, callback) {
    async.series(
        [async.apply(interfaceDown, iface), async.apply(interfaceUp, iface)],
        callback
    );
}
...
function listNetworks(callback) {
    exec(
        replaceInCommand(commands.wpaList, { wifiInterface: currentInterface }),
        function(err, stdout) {
